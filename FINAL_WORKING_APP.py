...
Guaranteed to work with Chrome extension for Murf API Challenge submission
"""
...
class MurfTTSService:
    """Murf TTS Service with text chunking"""
    ...
    def chunk_text(self, text: str, max_size: int = 3000) -> list:
        """Split text into chunks for Murf API (3000 char limit)"""
...
class SocialCastHandler(BaseHTTPRequestHandler):
    """HTTP handler for SocialCast API"""
    ...
    def do_POST(self):
        ...
        elif path == "/digest/generate":
            self.handle_digest_generate()
        else:
            self.send_error(404, "Not Found")
    ...
    def handle_digest_generate(self):
        """Handle daily digest generation"""
        try:
            content_length = int(self.headers['Content-Length'])
            post_data = self.rfile.read(content_length)
            request_data = json.loads(post_data.decode('utf-8'))
            ...
            if len(posts_storage) == 0:
                ...
                # Create digest audio
                digest_text = f"Daily digest for the last {hours} hours. "
                for i, post in enumerate(posts_storage[-5:], 1):  # Last 5 posts
                    digest_text += f"Post {i}: {post.get('summary', 'No content')}. "
                
                voice_config = {"voice_id": "en-US-amara", "style": "Conversational"}
                audio_path = tts_service.generate_audio(digest_text, voice_config)
                
                response = {
                    "message": f"Daily digest generated for last {hours} hours",
                    "audio_url": audio_path,
...
def run_server(port=8000):
    ...
    print("   POST /digest/generate - Generate daily digest")
    print("   PUT  /voice/config - Update voice configuration")
